...
%Memory main effecct bins
ERP = pop_binoperator(ERP, ...
                      {'b109 = (b8+b12+b24+b28)/4 label main_New', ...
                       'b110 = (b9+b13+b25+b29)/4 label main_Old'});
ERP = pop_binoperator(ERP, ...
                      {'b111 = b110-b109 label diff_main_Old-New'}); 
...
%Save filtered set
ERP = pop_savemyerp(ERP, ...
                    'erpname', sprintf('EmCon_GM%s_%dsubs_%dHz', subset{i}, length(ERP.workfiles), lp_filt), ...
                    'filename', sprintf('EmCon_GM%s_%dsubs_%dHz.erp', subset{i}, length(ERP.workfiles), lp_filt), ... 
                    'filepath', fullfile(main_dir, 'ERPsets', 'GM'), 'Warning', 'on');
ALLERP(CURRENTERP) = ERP;
